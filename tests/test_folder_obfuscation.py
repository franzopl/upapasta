...
class TestFolderObfuscation(unittest.TestCase):
    ...
    def test_folder_obfuscation_workflow(
        self, mock_managed_popen: MagicMock, mock_find_nyuu: MagicMock, mock_check_creds: MagicMock
    ):
        mock_proc = MagicMock()
        ...
        self.assertTrue(
            self.test_dir.exists(), "A pasta original deve continuar existindo (hardlinks)."
        )
        ...
        self.assertEqual(
            self.test_file.stat().st_ino,
            original_inode,
            "O inode do arquivo original não deve mudar.",
        )
        ...
        self.assertIsNotNone(
            obf_base, "Não foi possível encontrar o nome ofuscado da raiz no mapa."
        )
        ...
        self.assertTrue(
            obf_path.exists(),
            f"Links de ofuscação '{obf_base}' devem ter sido mantidos (keep_files=True).",
        )

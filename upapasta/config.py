...
def _write_full_env(env_file: str, values: dict[str, str]) -> None:
    ...
    lines = [
        ...
        "",
        "# Pasta específica para salvar os arquivos .nzb",
        f"NZB_OUT_DIR={v('NZB_OUT_DIR')}",
        "",
        "# Comportamento quando .nzb já existe: rename, overwrite, fail",
        f"NZB_CONFLICT={v('NZB_CONFLICT')}",
        "",
        "# *** Other Options ***",
        ...
        "",
        "# Args extras repassados ao pesto (ex: --rate=10M --verify)",
        f"PESTO_EXTRA_ARGS={v('PESTO_EXTRA_ARGS')}",
        "",
        "# Script executado após upload bem-sucedido",
        f"POST_UPLOAD_SCRIPT={v('POST_UPLOAD_SCRIPT')}",
        "",
        "# *** TMDb Metadata ***",
        ...
        "",
        "# Template de NFO (opcional)",
        f"NFO_TEMPLATE={v('NFO_TEMPLATE')}",
        "",
        "# Banner ASCII para o topo do NFO",
        f"NFO_BANNER={v('NFO_BANNER')}",
        "",
        ...
        "",
        "# URL para notificações via webhook",
        f"WEBHOOK_URL={v('WEBHOOK_URL')}",
        "",

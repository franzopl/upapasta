...
- **Ofuscação** (`--obfuscate`): cria hardlinks com nomes aleatórios para o input (ou rename se cross-device). O parpar gera PAR2 com nomes **reais** internamente (não aleatórios). `fix_nzb_subjects` restaura os nomes originais no NZB usando `obfuscated_map`. A estrutura interna é protegida por `_deep_obfuscate_tree` (renomeia arquivos/diretórios dentro de pastas). Resultado: subjects na Usenet são aleatórios, NZB tem nomes reais, PAR2 tem nomes reais → downloader recebe nomes reais, verificação PAR2 funciona ✓.

...
- **Ofuscação** (`--obfuscate`): cria hardlinks com nomes aleatórios para o input (ou rename se cross-device). O parpar gera PAR2 com nomes **reais** internamente. NZB mantém nomes aleatórios (nenhuma restauração). A estrutura interna é protegida por `_deep_obfuscate_tree` (renomeia arquivos/diretórios dentro de pastas). Resultado: NZB tem nomes aleatórios, PAR2 tem nomes reais internamente → downloader baixa com nomes aleatórios, PAR2 verifica e renomeia automaticamente para reais ✓. Simples e eficaz.
